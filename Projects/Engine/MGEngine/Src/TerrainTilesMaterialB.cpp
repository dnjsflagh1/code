...
namespace MG
{
    ...
    //---------------------------------------------------------------------
    void TerrainMaterialGeneratorB::SM2Profile::respondOptions(Bool immediately, Bool isSimple)
    {
    ...
    //---------------------------------------------------------------------
    void TerrainMaterialGeneratorB::SM2Profile::requestOptions(TerrainGroundTileRender* tileRender)
    {
    ...
    //---------------------------------------------------------------------
    uint8 TerrainMaterialGeneratorB::SM2Profile::getMaxLayers(TerrainGroundTileRender* tileRender) const
    {
    ...
    //---------------------------------------------------------------------
    MaterialPtr TerrainMaterialGeneratorB::SM2Profile::generate(TerrainGroundTileRender* tileRender)
    {

        TerrainSBGroundTileRender* bbTile = (TerrainSBGroundTileRender*)tileRender;
    ...
    //---------------------------------------------------------------------
    void TerrainMaterialGeneratorB::SM2Profile::updateParams(const MaterialPtr& mat, TerrainGroundTileRender* tileRender)
    {
        TerrainSBGroundTileRender* bbTile = (TerrainSBGroundTileRender*)tileRender;
    ...
    //---------------------------------------------------------------------
    void TerrainMaterialGeneratorB::SM2Profile::addFixedTechnique(const MaterialPtr& mat, TerrainSBGroundTileRender* tileRender, TechniqueType tt)
    {
    ...
    //---------------------------------------------------------------------
    void TerrainMaterialGeneratorB::SM2Profile::setTextureilter(TextureUnitState* tu)
    {
        IDisplay* display = EngineMain::getInstance().display();
    ...
    //---------------------------------------------------------------------
    void TerrainMaterialGeneratorB::SM2Profile::addWireFrameTechnique(
        const MaterialPtr& mat, TerrainSBGroundTileRender* tileRender, TechniqueType tt)
    {
    ...
    //---------------------------------------------------------------------
    void TerrainMaterialGeneratorB::SM2Profile::addNormalTechnique(
        const MaterialPtr& mat, TerrainSBGroundTileRender* tileRender, TechniqueType tt)
    {
        ...
        if (tt == HIGH_LOD )
        {
            ...
            {
                const StringVector& dsList = tileRender->getValidDiffuseSpecularTextureNameList();
            ...
            {
                const StringVector& nhList = tileRender->getValidNormalHeightTextureNameList();
        ...
        if (isShadowingEnabled(tt, tileRender))
        {
            ...
            PSSMShadowCameraSetup* pssmCam = tileRender->getTerrainManager()->getScene()->getShadowManager()->getActiveShadowMaker()->getPSSMShadowCameraSetup();

            if (pssmCam)
            {
                numTextures = pssmCam->getSplitCount();
            }
    ...
    //---------------------------------------------------------------------
    bool TerrainMaterialGeneratorB::SM2Profile::isShadowingEnabled(TechniqueType tt, TerrainSBGroundTileRender* tileRender) const
    {
    ...
    HighLevelGpuProgramPtr 
        ...
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt)
    {
    ...
    void TerrainMaterialGeneratorB::SM2Profile::ShaderHelper::generateVertexProgramSource(
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, StringUtil::StrStreamType& outStream)
    {
    ...
    void TerrainMaterialGeneratorB::SM2Profile::ShaderHelper::defaultVpParams(
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, const HighLevelGpuProgramPtr& prog)
    {
    ...
    void TerrainMaterialGeneratorB::SM2Profile::ShaderHelper::updateParams(
        const SM2Profile* prof, const MaterialPtr& mat, TerrainSBGroundTileRender* tileRender)
    {
    ...
    void TerrainMaterialGeneratorB::SM2Profile::ShaderHelper::updateVpParams(
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, const GpuProgramParametersSharedPtr& params)
    {
        params->setIgnoreMissingParams(true);
    ...
    //---------------------------------------------------------------------
    String TerrainMaterialGeneratorB::SM2Profile::ShaderHelper::getRGBAChannel(uint idx)
    {
        uint rem = idx % 4;
        switch(rem)
        {
    ...
    //---------------------------------------------------------------------
    String TerrainMaterialGeneratorB::SM2Profile::ShaderHelper::getXYZWChannel(uint idx)
    {
        uint rem = idx % 4;
        switch(rem)
        {
        ...
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, StringUtil::StrStreamType& outStream)
    {
        ...
        {
            outStream <<
                ",out float3 oUVMisc0 : TEXCOORD" << texCoordSet++ <<"\n";
        }else
        {
            outStream <<
        ...
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, StringUtil::StrStreamType& outStream)
    {
        ...
        {
            outStream <<
                " float3 uvMisc0 : TEXCOORD" << texCoordSet++ << "\n";
        }else
        {
            outStream <<
    ...
    void TerrainMaterialGeneratorB::SM2Profile::ShaderHelperCg::generateVpLayer(
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, uint layer, StringUtil::StrStreamType& outStream)
    {
        // nothing to do
    }
    //---------------------------------------------------------------------
        ...
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, uint layer, StringUtil::StrStreamType& outStream)
    {
        ...
        */
        ...
        const TerrainSBGroundTileRender::TextureLayerInstance* TextureLayerInstance = tileRender->getTextureLayer( layer );
        ...
        const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, StringUtil::StrStreamType& outStream)
    {
        ...
        if ( pssmCam )
        {
            uint numTextures = pssmCam->getSplitCount();
            ...
            {
                if (!i)
                    outStream << "	if (camDepth <= pssmSplitPoints." << ShaderHelper::getXYZWChannel(i) << ") \n";
                else if (i < numTextures - 1)
                    outStream << "	else if (camDepth <= pssmSplitPoints." << ShaderHelper::getXYZWChannel(i) << ") \n";
                else
                    outStream << "	else \n";
    ...
    uint TerrainMaterialGeneratorB::SM2Profile::ShaderHelperCg::generateVpDynamicShadowsParams(
        uint texCoord, const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, StringUtil::StrStreamType& outStream)
    {
    ...
    void TerrainMaterialGeneratorB::SM2Profile::ShaderHelperCg::generateFpDynamicShadowsParams(
        uint* texCoord, uint* sampler, const SM2Profile* prof, TerrainSBGroundTileRender* tileRender, TechniqueType tt, StringUtil::StrStreamType& outStream)
    {
        if (tt == HIGH_LOD)
