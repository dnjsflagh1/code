...
namespace SceneEditor
{
    ...
    void XmlStrProperty::saveXml()
    {
        if (mParentXmlElm)
        {
            ...
            XmlHelper::setAttribute( mParentXmlElm, Str8(attrValue.getChar8()), Str8(attrName.getChar8()) );                
        }
    }
    /******************************************************************************/
    //XmlGroupProperty
    ...
    void XmlPropertySet::clear()
    {
		...
		for each ( XmlGroupProperty^ myGroupProperty in m_subList )
		{
			delete myGroupProperty;
		}
	...
	//-----------------------------------------------------------------------------
	void XmlPropertySet::addSubGroupProperty(char* group_name, char* attr_name, char* attr_value)
	{
		String^ groupName = TypeConverter::Char8ToStr( group_name );
		String^ attrName = TypeConverter::Char8ToStr( attr_name );
		String^ attrValue = TypeConverter::Char8ToStr( attr_value );

		XmlStrProperty^ subStrProperty = nullptr;
		XmlGroupProperty^ subGroupProperty = nullptr;
		
		for each ( XmlGroupProperty^ myGroupProperty in m_subList )
		{
			if ( !myGroupProperty->Name->CompareTo(groupName) )
			{
				subGroupProperty = myGroupProperty;

				for each ( XmlStrProperty^ myStrProperty in subGroupProperty->mStrPropList )
				{
					if ( !myStrProperty->Name->CompareTo(attrName) )
					{
						subStrProperty = myStrProperty;
						break;
					}
				}
			}
		...
		if ( !subStrProperty )
		{
			...
			{
				array<Attribute^>^ attrs            = gcnew cli::array< Attribute^ >(0) {};
				subGroupProperty     = gcnew XmlGroupProperty(groupName, attrs);

				m_subList->Add(subGroupProperty);
				mCollection->Add(subGroupProperty);
			}

			array<Attribute^>^ descs = gcnew cli::array< Attribute^ >(0) {};
			subStrProperty = gcnew XmlStrProperty(NULL, attrName, attrValue, descs);

			subGroupProperty->mStrPropList->Add( subStrProperty );
			subGroupProperty->mGroup->mCollection->Add( subStrProperty );
		}
		
		if ( subStrProperty )
		{
			subStrProperty->mStrValue = attrValue;
		}
	}
	//-----------------------------------------------------------------------------
	std::string XmlPropertySet::getSubGroupProperty(char* group_name, char* attr_name)
	{
		std::string emptyStr;
		...
		for each ( XmlGroupProperty^ myGroupProperty in m_subList )
		{
			if ( !myGroupProperty->Name->CompareTo(groupName) )
			{
				...
				for each ( XmlStrProperty^ myStrProperty in subGroupProperty->mStrPropList )
				{
					...
					{
						subStrProperty = myStrProperty;
						return StrToChar8(subStrProperty->mStrValue).getChar8();
					}
				}
			}
		}
