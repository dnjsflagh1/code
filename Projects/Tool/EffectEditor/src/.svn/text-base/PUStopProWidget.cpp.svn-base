...
PUStopProWidget::PUStopProWidget( QWidget *parent /*= 0*/, Qt::WFlags flags /*= 0*/ )
: QWidget(parent)
, mPropertyBrw(NULL)
, mDoubleManager(NULL)
...
PUStopProWidget::~PUStopProWidget()
{
...
void PUStopProWidget::loadElementProps( MG::IEffectObjectXmlFormat* format )
{
	clearElementProps();
	...
	if (objtype != EOT_PU_STOP_EVENT)
		return;

	PU_Stop_ProSet();
}

void PUStopProWidget::clearElementProps()
{
	mCurrObjectXmlFormat = NULL;
	mPropertyBrw->clear();
...
void PUStopProWidget::createWidget()
{
...
void PUStopProWidget::PU_Stop_ProSet()
{
...
void PUStopProWidget::setSeletedEffectIndex( QtProperty *pro, Str name )
{
...
void PUStopProWidget::enumValueChanged( QtProperty *pro, int index)
{
	...
	IPUStopEffectEvent* pustopevent = 
		dynamic_cast<IPUStopEffectEvent*>(effectevent);

	QString proname = pro->propertyName();
	// 修改值
	if (StrCmp(proname.toStdString().c_str(), "PU Name") == 0)
	{
		pustopevent->setPUName(mEffectList[index].toStdString());
	}
...
void PUStopProWidget::doubleValueChanged( QtProperty *pro, double dvalue)
{
	QTreeWidgetItem* seletedItem = 
		WindowSet::main_win->getExComponentWidget()->getEventWidget()->getCurrentItem();
