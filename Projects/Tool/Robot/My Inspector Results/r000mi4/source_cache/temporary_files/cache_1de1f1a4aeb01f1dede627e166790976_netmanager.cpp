...
I32 NetManager::createDriver(	CChar* name, 
    ...
    //-----------------------------------------------------------------------------
    NetSendDataBufferPtr NetManager::getSendDataBuffer(I32 handle, U32 buffSize)
    {
        NetSendDataBufferPtr dataBuffObj;
        NetDriver* driver = getActiveDriver( handle );
        if ( driver )
        {
            dataBuffObj = driver->getSendDataBuffer(buffSize);
        }
        return dataBuffObj;
    }
