...
namespace MG
{
	...
	Bool ChatNetPacketProcesser::processFrontServerPacket( CClient* client,NetEventRecv* packet )
	{
	...
	Bool ChatNetPacketProcesser::processMapServerPacket( CClient* client,NetEventRecv* packet )
	{
		/*GameNetPacketData* data = (GameNetPacketData*)(packet->getData());
		if ( packet->getChannel() == GNPC_CHAT )
		{
	...
	void ChatNetPacketProcesser::onRecvTwoPersonChatContentFromChatServer( CClient* client,PT_CHAT_A2C_TWO_PERSON_DATA* data )
	{
  ...
  //      recdata->dataSize = data->dataSize;
	...
	void ChatNetPacketProcesser::onRecvChatResultFromFrontServer( CClient* client, PT_CHAT_F2C_RESULT_DATA* data )
	{
	...
	void ChatNetPacketProcesser::sendOneRegionChatToFrontServer( U32 channelId,RegionIdType regionId,REGION_OBJ_TYPE regionType,I32 size,I32 frontServerId )
	{
		// 获得一个发送【服务端】缓存
		...
		if ( packet )
		{
			...
			if ( sendData )
			{
                CChar16* senderClanName = L"robotchat";
				// 填充逻辑包
				sendData->channel = channelId;
				sendData->dataSize = size;
	...
	void ChatNetPacketProcesser::onRecvOneRegionContentFromMapServer( CClient* client, PT_CHAT_M2C_ONE_REGION_DATA* data )
	{
		//if (!(MessageManager::getInstance().getRecChatDataListSize() >= MAX_CHAT_CONTENT_ROW_COUNT))
		//{
		//	MessageManager::getInstance().addChatMsg(data->channel,data->senderClanName,(Char16*)data->data,(data->dataSize / sizeof(Char16))-1);
	...
	void ChatNetPacketProcesser::sendFullServerChatToFrontServer(U32 channelID,I32 size,I32 frontServerId) 
	{
		// 获得一个发送【服务端】缓存
		GameNetSendDataBufferPtr dataPtr = ClientNetApplyManager::getInstance().getSendDataBuffer( size + 24);
	...
	void ChatNetPacketProcesser::onRecvFullServerChatContentFromChatServer( CClient* client, PT_CHAT_A2C_FULL_SERVER_DATA* data )
	{
		//if (!(MessageManager::getInstance().getRecChatDataListSize() >= MAX_CHAT_CONTENT_ROW_COUNT))
		//{
		//	MessageManager::getInstance().addChatMsg(data->channel,data->senderClanName,(Char16*)data->data,(data->dataSize / sizeof(Char16))-1);
