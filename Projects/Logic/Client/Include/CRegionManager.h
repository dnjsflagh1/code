...
namespace MG
{
	...
	public:
		CRegionObject*		    getRegionObjectById(REGION_OBJ_TYPE regionType, RegionIdType regionId);

		
		std::map<MapDistributionIdType,	CMapDistribution*>*		getMapDistributionObjList();
		std::map<PlaceIdType,			CPlaceObject*>*			getPlaceObjList();
		std::map<InstanceIdType,		CTiroObject*>*			getTiroObjList();
		std::map<InstanceIdType,		CPveObject*>*			getPveObjList();
		std::map<InstanceIdType,		CHomeObject*>*			getHomeObjList();
