...
namespace MG
{
	...
	{
		ClientManager::getInstance().onConnected(id,address, connect);
	}

	//-----------------------------------------------------------------------------
	void FrontServerMain::onClientDisconnect(I32 id, INetConnect* connect)
	{
		ClientManager::getInstance().onDisconnect(id, connect);
	}

	//-----------------------------------------------------------------------------
	void FrontServerMain::onClientRecvPacket(I32 id,NetEventRecv* packet, INetConnect* connect)
	{
        ClientManager::getInstance().processPacket(id, packet, connect);
	}
