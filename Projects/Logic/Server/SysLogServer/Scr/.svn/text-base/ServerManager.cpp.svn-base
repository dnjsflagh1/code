...
namespace MG
{
	...
	Bool ServerManager::processPacket(I32 id, NetEventRecv* packet)
	{
        ...
        ServerLogNetPacketProcesser::getInstance().processSysLogServerPacket(id, packet);
    ...
    void ServerManager::boardcastNetPacketToServers( GameNetSendDataBufferPtr& packetPtr, I32 dataSize, GameNetType type )
    {
        ...
        while (front != back)
        {
            if ( (*front)->type == type )
            {
                SysLogServerMain::getInstance().getServerLauncher()->sendServer( packetPtr, dataSize, (*front)->netId );
            }
        }
