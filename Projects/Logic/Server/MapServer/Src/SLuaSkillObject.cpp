...
namespace MG
{
 ...
 //       if (NULL == skillObject) { DYNAMIC_ASSERT(0); return 0; }
	//	SCharacter* character = SWorldManager::getInstance().getCharacterFromMemory(skillObject->getOwnerType(), skillObject->getOwnerID(), skillObject->getOwnerIndex());
	//	if(!character)
	//	{
	//		return 0;
	//	}
 ...
 //   }
	...
	////-----------------------------------------------------------------------
	//int SLuaSkillObject::getSkillLevel(lua_State *L)
	//{
	//	SkillObject* skillObject  = LUA_RETRIEVE_OBJECT(SkillObject,SLuaSkillObject,L); 
	//	if (NULL == skillObject) { DYNAMIC_ASSERT(0); return 0; }
