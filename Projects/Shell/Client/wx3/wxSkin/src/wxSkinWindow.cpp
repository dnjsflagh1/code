...
void wxSkinWindow::SetFixedBitmap(const wxImage& fixedBitmap)
{
	bmp_fixedBitmap = fixedBitmap;
}

void wxSkinWindow::SetStateFlag(const int stateflag)
{
	m_stateFlag = stateflag;
}

void wxSkinWindow::ShowNormalSkin()
{
	m_stateFlag = 0;
	wxClientDC dc(this);
	int h, w;
	GetSize(&h,&w);
	if(bmp_normal.ISOK())
		dc.DrawBitmap(wxBitmap(bmp_normal.Scale(h, w)),0,0);
}
...
void wxSkinWindow::OnPaint(wxPaintEvent& e)
{
	...
	{	
		...
		if(bmp_fixedBitmap.ISOK())
			dcp.DrawBitmap(wxBitmap(bmp_fixedBitmap.Scale(h, w)),0,0,true);
		else if(m_stateFlag)
		{
			//不属于正常状态
			if(m_stateFlag == 1 && bmp_over.ISOK())
			{
				dcp.DrawBitmap(wxBitmap(bmp_over.Scale(h, w)),0,0,true);
				m_fontState = 1;
			}
			else if(m_stateFlag == 2 && bmp_state2.ISOK())
			{
				m_fontState = 2;
				dcp.DrawBitmap(wxBitmap(bmp_state2.Scale(h, w)),0,0,true);
			}
			else if(m_stateFlag == 3 && bmp_disabled.ISOK())
			{
				dcp.DrawBitmap(wxBitmap(bmp_disabled.Scale(h, w)),0,0,true);
			}
		}
		else if(m_bInside && bmp_over.ISOK())
		{
			m_fontState = 1;
			dcp.DrawBitmap(wxBitmap(bmp_over.Scale(h, w)),0,0,true);
		}
	...
	{
		if(bmp_fixedBitmap.ISOK())
			dcp.DrawBitmap(bmp_fixedBitmap,0,0,true);
		else if(m_stateFlag)
		{
			//不属于正常状态
			if(m_stateFlag == 1)
			...
			else if(m_stateFlag == 2)
			{
				m_fontState = 2;
				dcp.DrawBitmap(bmp_state2,0,0,true);
			}
			else if(m_stateFlag == 3)
			{			
