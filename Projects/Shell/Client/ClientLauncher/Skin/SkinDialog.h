...
public:
	...
	CSkinDialog (LPCTSTR lpszResourceName = NULL, UINT nIDResource = 0,
		LPCTSTR lpszFilename = NULL, CBitmap *pBitmap = NULL);

	// Destructor (just release the bitmap)
	~CSkinDialog () { ReleaseBitmap (); }
	void		SetRgnFileName(const char* lpFileName);
	// Bitmap
	BOOL		LoadBitmap (LPCTSTR lpszResourceName);					// Load from resource or file
	BOOL		LoadBitmap (UINT nIDResource);			// Load from resource
	BOOL		LoadBitmapFromFile(const char* lpszFileName,Memfile xFunc);
