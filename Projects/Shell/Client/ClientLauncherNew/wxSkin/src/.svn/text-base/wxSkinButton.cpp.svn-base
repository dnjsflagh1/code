...
wxSkinButton::wxSkinButton()
	: wxSkinWindow()
{
	m_fontColor = m_overFontColor = m_actFontColor = GetForegroundColour();
}
//Passing images
...
{	
	...
	{	int h, w;
		...
		if ( IsEnabled() )
			...
			else if (m_fontState == 1)
				dc.SetTextForeground(/*GetForegroundColour()*/ m_overFontColor);
			else
				dc.SetTextForeground(/*GetForegroundColour()*/ m_actFontColor);
		else
...
void wxSkinButton::ShowNormalSkin()
{
	m_stateFlag = 0;
	m_fontState = 0;
	wxClientDC dc(this);
