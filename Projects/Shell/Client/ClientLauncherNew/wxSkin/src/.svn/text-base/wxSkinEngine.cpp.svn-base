...
bool wxSkinEngine::ReassignControlSkin(const wxWindow* ptr, const wxString& label)
{
	...
	if(itControl != m_controlMap.end())
	{
		...
		{	
			...
			//xcl
			win->SetFixedBitmap(LoadSkinImage(m_skinfile+info.fixedBitmap));
			win->SetStateFlag(info.stateFlag);
			//-xcl
...
{	
	...
	switch(type)
	{
		...
		case TYPE_TEXT:
		{
			nodeName.Append(wxT("TEXT"));
		}
		break;
		case TYPE_BITMAP:
		{
			nodeName.Append(wxT("BITMAP"));
		}
		break;
	}
	...
	{	win->SetDisabledSkin(wxNullImage);
		...
		//xcl
		win->SetFixedBitmap(wxNullImage);
		win->SetStateFlag(0);
		//-xcl
	}
}
...
bool wxSkinEngine::Load(const wxString& file, const wxString& subFile)
{
	wxFileSystem system;
...

	{
		wxLogNull logNo;
		filePrefix = file+wxT("#zip:");
		fsfile = system.OpenFile(filePrefix+subFile);		// replace extern file.
	...
	while(node)
	{
		...
		else if (nodeName == wxT("Texts"))
			ParseInnerAttributes(TYPE_TEXT,node);
		else if (nodeName == wxT("Bitmaps"))
			ParseInnerAttributes(TYPE_BITMAP,node);
	...
	while(thisNode)
	{
		...
		switch(type)
		{
			...
			{	
				...
				//xcl
				wxString stateflag = thisNode->GetPropVal(wxT("stateflag"),wxT("normal"));
				if(stateflag == wxT("over"))
					controlInfo.stateFlag = 1;
				else if(stateflag == wxT("act"))
					controlInfo.stateFlag = 2;
				else if(stateflag == wxT("dis"))
					controlInfo.stateFlag = 3;
				//-xcl
				int i=0;
			}	
			break;
			...
			case TYPE_BITMAP:
			{
				controlInfo.fixedBitmap = thisNode->GetPropVal(wxT("bitmap"),wxT(""));
			}
			break;
		}
