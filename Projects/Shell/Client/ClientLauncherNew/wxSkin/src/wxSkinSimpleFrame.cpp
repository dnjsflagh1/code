...
void wxSkinSimpleFrame::DrawImage(wxSize &csize)
{
	m_drawImg = true;		
	m_customSize = csize;
}

void wxSkinSimpleFrame::SetSubImg(wxImage &img, wxPoint &point)
{
	m_customImg = img;
	m_customPoint = point;
}

void wxSkinSimpleFrame::SetMainImg(wxImage &img, wxPoint &point)
{
	m_customImg1 = img;
	m_customPoint1 = point;
}
...
void wxSkinSimpleFrame::RecreateSizer()
{
	...
	{	wxSizer* topSizer = GetSizer();
		...
		{	if(topSizer->GetItem((size_t)0) && topSizer->GetItem((size_t)0)->IsSpacer())
				topSizer->GetItem((size_t)0)->SetSpacer(wxSize(rect_clientarea.y,rect_clientarea.y));
			...
			{	
				lineSizer->GetItem((size_t)0)->SetSpacer(wxSize(rect_clientarea.x,rect_clientarea.x));
			}
			if(lineSizer && lineSizer->GetItem((size_t)2) && lineSizer->GetItem((size_t)2)->IsSpacer())
			{	
				int s = m_bmpSkin.GetWidth()-rect_clientarea.x-rect_clientarea.width;
				lineSizer->GetItem((size_t)2)->SetSpacer(wxSize(s,s));
			}
			...
			{	int s = m_bmpSkin.GetHeight()-rect_clientarea.y-rect_clientarea.height;
				topSizer->GetItem((size_t)2)->SetSpacer(wxSize(s,s));
			}
		}
		Layout();
	}
...
void wxSkinSimpleFrame::OnPaint(wxPaintEvent& event)
{
	...
	dc.DrawText(_("think"), 1905, 850);

	if (m_drawImg)
	{
		wxBitmap bmp(m_customImg.Rescale(m_customSize.x, m_customSize.y));
		dc.DrawBitmap(bmp,m_customPoint);
	}
}
#ifdef __WXGTK__
