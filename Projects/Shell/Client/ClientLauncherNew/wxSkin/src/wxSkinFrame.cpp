...
void wxSkinFrame::RecreateSizer()
{
	...
	{	
		...
		{	if(topSizer->GetItem((size_t)0) && topSizer->GetItem((size_t)0)->IsSpacer())
				topSizer->GetItem((size_t)0)->SetSpacer(wxSize(skin_top.GetHeight(),skin_top.GetHeight()));
			...
			{	
				lineSizer->GetItem((size_t)0)->SetSpacer(wxSize(skin_left.GetWidth(),skin_left.GetWidth()));
			}
			if(lineSizer && lineSizer->GetItem((size_t)2) && lineSizer->GetItem((size_t)2)->IsSpacer())
			{	
				lineSizer->GetItem((size_t)2)->SetSpacer(wxSize(skin_right.GetWidth(),skin_right.GetWidth()));
			}

			if(topSizer->GetItem((size_t)2) && topSizer->GetItem((size_t)2)->IsSpacer())
			{	topSizer->GetItem((size_t)2)->SetSpacer(wxSize(skin_bottom.GetHeight(),skin_bottom.GetHeight()));
			}
		}
		Layout();
	}
