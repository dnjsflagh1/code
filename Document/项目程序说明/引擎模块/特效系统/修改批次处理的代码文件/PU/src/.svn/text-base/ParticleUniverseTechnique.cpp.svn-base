...
namespace ParticleUniverse
{
	...
	void ParticleTechnique::setRenderer(const String& rendererType)
	{
		...
		{
#ifdef CN_MODIFY
			mRenderer = ParticleRendererManager::getSingletonPtr()->createRenderer(this, getParentSystem()->getRenderGroupID(), rendererType);
#else
			mRenderer = ParticleSystemManager::getSingletonPtr()->createRenderer(rendererType);
#endif
			mRenderer->setParentTechnique(this);
#ifdef CN_MODIFY
			ParticleRendererManager::getSingletonPtr()->setRendererInitialised(mRenderer, false);
#else
...
#else
		ParticleRendererManager::getSingletonPtr()->removeRenderer(renderer);
#endif
	}
	//-----------------------------------------------------------------------
	void ParticleTechnique::destroyRenderer(void)
	{
		...
		if (mRenderer)
		{
			ParticleRendererManager::getSingletonPtr()->destroyRenderer(mRenderer);
		}
#endif
	}
	//-----------------------------------------------------------------------
	...
	{
#ifdef CN_MODIFY
		if (mRenderer && ParticleRendererManager::getSingletonPtr()->isRendererInitialised(mRenderer))
#else
...
#else
		ParticleRendererManager::getSingletonPtr()->_prepareRenderer(mRenderer);
#endif
	}
	//-----------------------------------------------------------------------
...
#else
		ParticleRendererManager::getSingletonPtr()->_unprepareRenderer(mRenderer);
#endif
	}
	//-----------------------------------------------------------------------
